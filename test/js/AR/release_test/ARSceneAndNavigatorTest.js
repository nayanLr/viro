...
let pointCloudDict = {
  ...
  imageScale: [0.1, 0.1, 0.1],
  maxPoints: 50,
};

var projectPoint = [1, 1, -4];
var unprojectPoint = [200, 500, 0.1];
...
var testARScene = createReactClass({
  ...
  render: function () {
    return (
      ...
      >
        ...
        <ViroText
          position={polarToCartesian([6, 0, 40])}
          text={
            "Unproject point(200, 500, .1)==>(" +
            this.state.unprojectedPoint[0] +
            "," +
            this.state.unprojectedPoint[1] +
            "," +
            this.state.unprojectedPoint[2] +
            ")"
          }
          style={styles.instructionText}
          onClick={this._unprojectPoint}
          transformBehaviors={["billboard"]}
        />
        <ViroText
          position={polarToCartesian([6, 0, 30])}
          text={
            "Projected point(1, 1, -4)==>(" +
            this.state.projectedPoint[0] +
            "," +
            this.state.projectedPoint[1] +
            "," +
            this.state.projectedPoint[2] +
            ")"
          }
          style={styles.instructionText}
        ...
        />
        <ViroText
          position={polarToCartesian([6, 0, 20])}
          text={this.state.trackingState}
          style={styles.instructionText}
          onClick={this._startStopRecording}
          transformBehaviors={["billboard"]}
        />
        <ViroText
          position={polarToCartesian([6, 0, 10])}
          text={"Recording? " + (this.state.isRecording ? "Yes" : "No")}
          style={styles.instructionText}
          onClick={this._startStopRecording}
          transformBehaviors={["billboard"]}
        />
        <ViroText
          position={polarToCartesian([6, 0, 0])}
        ...
        <ViroText
          position={polarToCartesian([6, 0, -10])}
          text={"saveToCamera? " + (this.state.saveToCamera ? "Yes" : "No")}
          style={styles.instructionText}
          onClick={this._flipSaveToCamera}
          transformBehaviors={["billboard"]}
        />
        <ViroText
          position={polarToCartesian([6, 0, -20])}
          text={
            "World Origin State: " +
            (this.state.worldOriginChanged ? "[0,0,1]" : "[0,0,0]")
          }
          style={styles.instructionText}
          onClick={this._changeWorldOrigin}
          transformBehaviors={["billboard"]}
        />
        <ViroText
          position={polarToCartesian([6, 0, -30])}
          text={"Evict Image From Cache (Android)"}
          style={styles.instructionText}
          onClick={this._evictFromCache}
          transformBehaviors={["billboard"]}
        />
        ...
        {/* ViroARScene tests */}
        <ViroText
          position={polarToCartesian([6, 30, 10])}
          text={"Initialized? " + (this.state.initialized ? "Yes" : "No")}
          style={styles.instructionText}
          transformBehaviors={["billboard"]}
        />
        <ViroText
          position={polarToCartesian([6, 30, 0])}
          text={this.state.ambientLightText}
          style={styles.ambientLightText}
          transformBehaviors={["billboard"]}
          ref={(component) => {
            this._ambientLightText = component;
          }}
        />
        <ViroText
          ref={(component) => {
            this.pointCloudPointsText = component;
          }}
          position={polarToCartesian([6, 30, -20])}
          text={"# of PointCloud points: " + this.state.pointCloudPoints}
          style={styles.instructionText}
          transformBehaviors={["billboard"]}
        />
        <ViroText
          position={polarToCartesian([6, 30, -30])}
          text={
            this.state.displayPointCloud
              ? "Hide Point Cloud"
              : "Show Point Cloud"
          }
          style={styles.instructionText}
          transformBehaviors={["billboard"]}
          onClick={() => {
            this.setState({ displayPointCloud: !this.state.displayPointCloud });
          }}
        />
        <ViroText
          position={polarToCartesian([6, 30, -40])}
          text={"Toggle Autofocus/Quality"}
          style={styles.instructionText}
          transformBehaviors={["billboard"]}
          onClick={() => {
            this.props.arSceneNavigator.viroAppProps.toggleAutofocus();
        ...
        <ViroText
          ...
          text={"Next test"}
          style={styles.instructionText}
          onClick={this._goToNextTest}
          transformBehaviors={["billboard"]}
        />
        <ViroText
          position={polarToCartesian([6, -30, -15])}
          text={"Release Menu"}
          style={styles.instructionText}
          onClick={() => {
    ...
    this.setState(
      ...
      () => {
        this.props.arSceneNavigator
          .takeScreenshot("testScreenshot", this.state.saveToCamera)
          .then((retDict) => {
            if (!retDict.success) {
              if (
                retDict.errorCode == ViroConstants.RECORD_ERROR_NO_PERMISSION
              ) {
                console.log("[JS] screenshot error, had no permissions!");
              } else {
                console.log(
                  "[JS] other screenshot error: " + retDict.errorCode
                );
              }
            }
  ...
  _startRecording() {
    ...
    this.setState(
      ...
      () => {
        this.props.arSceneNavigator.startVideoRecording(
          "testVideo",
          this.state.saveToCamera,
          (errorCode) => {
            console.log("[JS] onError callback errorCode: " + errorCode);
          }
        );
      }
    );
  ...
  _goToNextTest() {
    this.props.arSceneNavigator.replace("ARDragTest", {
      scene: require("./ARDragTest"),
    });
  },
});
